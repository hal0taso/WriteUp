...
int main()
{
  ...
  { return l + r; };
  LazySegmentTree<ll> st(n + 1, 0LL, 0LL, maxop, addop, addop);

  st.build(a);

  for (int i = 0; i < m; i++)
  {
    ll cntb = st.fold(b[i], b[i] + 1);
    st.update(b[i], b[i] + 1, -cntb);
    ll mul = cntb / n;
    ll mod = cntb % n;
    st.update(0, n, mul);
    if (b[i] + mod < n)
      st.update(b[i] + 1, b[i] + mod + 1, 1);
    else
    {
      st.update(b[i] + 1, n, 1);
      st.update(0, (b[i] + mod + 1) % n, 1);
    }
  }
  for (int i = 0; i < n; i++)
  {
