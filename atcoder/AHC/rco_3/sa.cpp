...
void update_nearest(vector<vector<int>> &a, int i, int j, UnionFind &uf)
{
  int u, v;
  // puts("called");
  for (int d : {-1, 1})
...
struct STATE
{
  vector<vector<int>> lbA;
  vector<vector<int>> curA;
  int n;
  int cost;
  STATE(int _n, vector<vector<int>> a)
  {
    lbA.assign(_n, vector<int>(_n, 0));
    curA.assign(_n, vector<int>(_n, 0));
    n = _n;
    REP(i, _n)
    REP(j, _n)
    lbA[i][j] = a[i][j], curA[i][j] = a[i][j];
    cost = 0;
  }
  STATE(){};
};
...
void modify(STATE &state)
{
  ...
  // do
  // {
  int row = (rand() % (N - WINDOW + 1));
  int col = (rand() % (N - WINDOW + 1));
  int target = (rand() % 9) + 1;
  for (int i = 0; i < WINDOW; i++)
  {
    for (int j = 0; j < WINDOW; j++)
    {
      if (state.lbA[i + row][j + col] <= target)
      {
        state.curA[i + row][j + col] = target;
        state.cost += target - state.curA[i + row][j + col];
      }
    }
  }
  // if (state.cost >= UPLMT)
...
int calc_score(STATE &state)
{
  ...
  REP(i, n)
  {
    REP(j, n)
    {
      update_nearest(state.curA, i, j, uf);
    }
  }
  if (state.cost > UPLMT)
    return -1;
  REP(i, n)
  {
    REP(j, n)
    {
      int v = cell(i, j);
      int rv = uf.root(v); // 収穫の操作は連結集合のrootだけにしたい
      if (!used[rv] && uf.size(v) >= state.curA[i][j])
      {
        // 収穫できる
        used[rv] = true;
        ans.push({state.curA[i][j] * uf.size(rv), rv});
      }
    }
  }

  int cnt = M - state.cost;
  int ret = 0;
  while (cnt > 0)
  {
    pair<int, int> cand = ans.top();
    ret += cand.first;
    cnt--;
    ans.pop();
  }
  return ret - max(state.cost - UPLMT, 0);
}

// 焼きなまし法
void sa(int n, int m, vector<vector<int>> &a)
{
  STATE state(n, a);
  // puts("INIT");
  init(state);
  int init_score = calc_score(state);
  // cout << "INIT DONE: score: " << init_score << '\n';
  double start_temp = 9 * WINDOW * WINDOW, end_temp = 1; // 適当な値を入れる（後述）
  double start_time = getTime();                         // 開始時刻
  // cout << "START at " << start_time << '\n';
...
int main()
{
  ...
  origA2.assign(N, vector<int>(N, 0));
  ...
  //     int v = cell(i, j);
  //     int rv = uf.root(v); // 収穫の操作は連結集合のrootだけにしたい
  //     if (!used[rv] && uf.size(v) >= A[i][j])
  //     {
  //       // 収穫できる
  //       used[rv] = true;
  //       ans.push({A[i][j] * uf.size(rv), rv});
  //     }
  //   }
  // }
  ...
  //   pair<int, int> cand = ans.top();
  //   ret += cand.first;
  //   int x = (cand.second - 1) / N, y = (cand.second - 1) % N;
  //   cout << 2 << ' ' << x << ' ' << y << '\n';
  //   cnt--;
  //   ans.pop();
  // }
